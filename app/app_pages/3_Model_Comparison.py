...
st.subheader(":material/leaderboard: Leaderboard", anchor=False)
...
st.subheader(":material/bar_chart: Accuracy comparison", anchor=False)
acc_df = pd.DataFrame(comparison)[["model", "accuracy"]].sort_values("accuracy")
st.bar_chart(acc_df, x="model", y="accuracy")

st.space("small")

st.subheader(":material/note_alt: Architecture notes", anchor=False)
notes = [
...
for name, detail, suitability in notes:
